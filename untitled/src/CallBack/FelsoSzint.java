...
public class FelsoSzint {
    private class HibaCallBackImplementacio implements HibaCallBack{
    ...
    boolean hibaFlag = false;
    private int[] szamok;
    private FelsoSzintService service;
    private HibaCallBack hbc = new HibaCallBackImplementacio();
    ...
    public int getArrayGCD() {
        ...
        int gcd = service.gcd(szamok[0], szamok[1], hbc);
        if(hibaFlag) {
            hibaFlag = false;
            return getArrayGCD_Default();
        }
        for(int i = 2; i < szamok.length; i++) {
            gcd = service.gcd(gcd, szamok[i], hbc);
            if(hibaFlag) {
                hibaFlag = false;
                return getArrayGCD_Default();
            }
